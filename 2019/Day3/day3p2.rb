...
def moving(wire)
	...
	wire.each do |move|
		...
		(0..number).each do |i|
			point = [x+px*i,y+py*i]
			positions.add point
			steps[point] = pas + i unless steps.include? point
		end
		x += px * number
		y += py * number
		pas += number
	end
	return [positions.delete([0,0]), steps]
end
...
wire1 = wire1.split(",")
wire2 = wire2.split(",")
chemin1, step1 = moving(wire1)
chemin2, step2 = moving(wire2)
puts (chemin1 & chemin2).map{|x| step1[x] + step2[x]}.min